...
<html>
    ...
    <div class="example-card" >
        ...
        <br>
        <?php
        $sql ="select * from citas c where c.idcitas =". $_GET["citas"];
        $resultado = mysqli_query($conexion,$sql);
        while($fila = mysqli_fetch_array($resultado)){
            $fecha = $fila["fechacita"];
            $hora = $fila["horacita"];
            $monto = $fila["montocita"];
            $montopago= $fila["montopagar"];
            $tratamiento = $fila["tratamientocita"];
            $cliente= $fila["cliente"];
            $dentista = $fila["dentista"];
        }
 	    ?>
        <div class = "example-form">
            <form name="datos" method="post" action="update_cita.php" accept-charset="UTF-8">
            <input type="hidden" name ="citas" value="<?= $_GET["citas"];?>">
                <div class="field" tabindex="1">
                    <label for="idcitas">ID de la Cita</label>
                    <input type='number' name='idcitas'  minlength='6' value='<?=$_GET["citas"]?>' readonly>
                </div>
                <div class="field" tabindex="2">
                    <label for="fecha">Fecha </label>
                    <input name="fecha" type="text" value='<?=$fecha ?>' required>
                </div>
                <div class="field" tabindex="3">
                    <label for="hora">Hora</label>
                    <input name="hora" type="text" value='<?=$hora?>' required>
                </div>
                <div class="field" tabindex="4">
                    <label for="monto">Monto</label>
                    <input name="monto" type="text" value='<?=$monto ?>' required>
                </div>
                <div class="field" tabindex="5">
                    <label for="tratamiento">Tratamiento</label>
                    <input name="tratamiento" type="text"  value='<?=$tratamiento ?>' required>
                </div>
                <div class="field" tabindex="6">
                    <label for="cliente">Cliente</label>
                    <input name="cliente" type="text" value='<?=$cliente ?>' required>
                </div>
                        ...
                        <option value="" selected></option>
                            <?php
                                ...
                                while($doctor = mysqli_fetch_array($doctores)){
                                    $strsel="";
                                    if($dentista == $doctor["dpiden"]){
                                        $strsel = "selected";
                                        echo '<option value = "' . $doctor["dpiden"] . '"'. $strsel .'>' . $doctor["nombresden"] . '</option>';
                                    }
                ...
                <a href = "http://localhost/ClinicaDental/interior/barra.php?pa=buscar_pacientes"><button type="button">Regresar</button></a>
            </form>
